...
async function refreshLive() {
    const baseUrl = byId("apiBase").value.trim();
    if (!baseUrl) {
        setApiStatus("Set API Base URL first.", "error");
        return;
    }

    try {
        const [stats, orders, trades] = await Promise.all([
            fetchJSON(baseUrl, "/stats"),
            fetchJSON(baseUrl, "/orders"),
        ...
        setText("lastTradesStamp", `updated ${nowStamp()}`);
        setText("lastOrdersStamp", `updated ${nowStamp()}`);
        setApiStatus(`Live API connected at ${nowStamp()}`, "ok");
        buildCombinedSummary();
    } catch (error) {
        console.error(error);
        setApiStatus(`API refresh failed: ${error.message}`, "error");
    }
}
